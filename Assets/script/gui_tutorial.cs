...
// Use this for initialization
	void Start () {
		...
		if(language=="english"){
			...
			label_num.guiText.text="クリック回数: 0";
			label_error.guiText.text="エラーをクリック: 0";
		}
		...
		if(language=="english"){
			...
			if(platform_type=="PC"){
				instruction[0]="物体を回転させるには、マウスの左ボタンを押しながら、マウスを動かして下さい。\n次のレベルに進むには、（次へ）ボタンを押してください";
				instruction[1]="マウスの中央のホイールを上向きに回して下さい。縮小するには、 逆の操作をして下さい。\n次のレベルに進むには、（次へ）ボタンを押してください";
				instruction[2]="キーボードの赤ボタンを押しながら、マウスで画面をなぞって下さい。\n次のレベルに進むには、（次へ）ボタンを押してください";
				instruction[3]="リセットボタンを押すと、物体が初めの位置に戻ります。\n次のレベルに進むには、（次へ）ボタンを押してください";
				instruction[4]="左側の物体と右側の物体の違いを探して下さい。違う場所を見つけたら。その場所をダブルクリックしてください。";
				instruction[5]="練習が終わったら、次へボタンを押して下さい。間違い探しの練習に進みます。";
			}
			else{
				instruction[0]="物体を回転させるには、画面をなぞって下さい。\n次のレベルに進むには、（次へ）ボタンを押してください";
				instruction[1]="拡大するには、 画面の物体上に２本の指を置いて広げて下さい\n次のレベルに進むには、（次へ）ボタンを押してください";
				instruction[2]="物体の位置を変えるには、２本の指で画面をなぞって下さい\n次のレベルに進むには、（次へ）ボタンを押してください";
				instruction[3]="リセットボタンを押すと、物体が初めの位置に戻ります。\n次のレベルに進むには、（次へ）ボタンを押してください";
				instruction[4]="左側の物体と右側の物体の違いを探して下さい。違う場所を見つけたら。その場所をダブルクリックしてください。";
				instruction[5]="練習が終わったら、次へボタンを押して下さい。間違い探しの練習に進みます。";
			}
	...
	IEnumerator GUI_Update(){
		...
		while(!level_end){
			if(language=="english"){
				...
				label_difference.guiText.text="残りの違い: "+(total_difference-score)+"    違いを見つけた: "+score;
				label_num.guiText.text="クリック回数: "+click_count;
				label_error.guiText.text="エラーをクリック: "+error_click;
			}
			yield return new WaitForSeconds (0.1f);
		}
		...
		if(score==total_difference){
			if(language=="english"){
				...
				label_message.guiText.text="レベルクリア!\nあなたが"+(time_completed).ToString("f1")+"秒で完了\n次のボタン押し続けると";
				label_difference.guiText.text="残りの違い: 0    違いを見つけた: "+total_difference;
			}
			//~ Rect rect_box=label_message.guiText.GetScreenRect ();
			//~ gui_box.guiTexture.pixelInset=new Rect(-rect_box.width/2-10, -rect_box.height/2-10, rect_box.width+20, rect_box.height+20);
		}
