...
public class gui_gameplay : MonoBehaviour {
	...
	private bool using_mouse=false; //a flag indicate the operating mode, prevent conflict when doing double click/tap
	...
	private float double_tap_interval=1.2f;
	private float double_click_interval=0.7f;
...
// Use this for initialization
	...
	//~ void OnGUI(){
		//~ GUI.Label(new Rect(20, Screen.height*0.78f, 500, 500), debug_message);
	//~ }
		...
		if(!level_end && level_started){
			if(!using_mouse){
				if(iPhoneInput.touchCount==1 && !touched_1){
					touched_1=true;
					if ((Time.time - doubleTapStart) < double_tap_interval){
						debug_message="double tap!";
						OnSelectDifference(iPhoneInput.GetTouch(0).position);
						doubleTapStart=-1;
					}
					else{
						debug_message="single tap! time since last tap = "+(Time.time - doubleTapStart).ToString("f3");
						doubleTapStart = Time.time;
					}
				}
				else if(iPhoneInput.touchCount!=1) touched_1=false;
				//~ else debug_message="no input!";
			}
			else if(using_mouse){
				if(Input.GetMouseButtonDown(0)){
					//~ print("mousedown");
					if ((Time.time - doubleClickStart) < double_click_interval){
		...
		if (button_reset.guiText.HitTest( pos )){
			...
			if(next_level!="") {
				if(next_level=="main_menu")
					...
					StartCoroutine(Load_Next_Level(3)); //next_level
				if(score==total_difference) Record("completed");
				else Record("skipped");
			}
			else {
	...
	void Record(string status){
		...
		if(record_data){
			...
			object_touch_control otc = gameObject.GetComponent<object_touch_control>() as object_touch_control;
			
			string flick_count=Mathf.Max(omc.GetFlick(), otc.GetFlick()).ToString();
			string zin_count=Mathf.Max(omc.GetZoomIn(), otc.GetZoomIn()).ToString();
			string zout_count=Mathf.Max(omc.GetZoomOut(), otc.GetZoomOut()).ToString();
			string pan_count=Mathf.Max(omc.GetPan(), otc.GetPan()).ToString();
			
			print(flick_count+"  "+zin_count+"  "+zout_count+"  "+pan_count);
			
			if(scoreMagr != null){
				if(status=="completed")
					scoreMagr.SaveScore(Application.loadedLevel.ToString(), click_count.ToString(), time_completed.ToString("f1"), error_array, reset_count.ToString(), status, flick_count, zin_count, zout_count, pan_count);
				else if(status=="skipped"){
					//~ temp=(Time.timeSinceLevelLoad-2.5f);
					//~ if(temp<0.0f) temp=0.0f;
					if(Time.timeSinceLevelLoad>time_given)
						scoreMagr.SaveScore(Application.loadedLevel.ToString(), click_count.ToString(), time_given.ToString("f1"), error_array, reset_count.ToString(), status, flick_count, zin_count, zout_count, pan_count);
					else
						scoreMagr.SaveScore(Application.loadedLevel.ToString(), click_count.ToString(), Time.timeSinceLevelLoad.ToString("f1"), error_array, reset_count.ToString(), status, flick_count, zin_count, zout_count, pan_count);
				}
			}
		}
	}
